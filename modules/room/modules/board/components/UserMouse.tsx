...
const UserMouse = ({ userId }: { userId: string }) => {
  ...
  return (
    ...
    >
      ...
      {msg && (
        <p className="absolute top-full left-5 max-h-20 max-w-[15rem] overflow-hidden text-ellipsis rounded-xl bg-zinc-900/90 p-2 px-4 text-white shadow-soft border border-border mt-2">
          {msg}
        </p>
      )}
      <p className="ml-2 font-bold text-xs bg-white/80 rounded px-2 py-1 shadow-soft border border-border mt-1">
